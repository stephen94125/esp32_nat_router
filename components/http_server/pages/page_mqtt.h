...
input[type='text'], input[type='password'], input[type='number'], select { width: 100%; background: rgba(22, 33, 62, 0.6); border: 1px solid rgba(0, 217, 255, 0.2); border-radius: 8px; color: #e0e0e0; padding: 0.75rem; font-size: 0.95rem; }\
...
@media (max-width: 600px) { body { padding: 0.5rem; } #container { padding: 1rem; } h1 { font-size: 1.25rem; } h2 { font-size: 1rem; } td:first-child { font-size: 0.8rem; width: 40%; } input[type='text'], input[type='password'], input[type='number'], select { font-size: 0.9rem; padding: 0.65rem; } .ok-button { font-size: 0.9rem; padding: 0.65rem 1.25rem; } }\
...
#define MQTT_CHUNK_SCRIPT "\
</div>\
<script>\
if (window.location.search.indexOf('reset=1') !== -1) {\
document.getElementById('container').style.display = 'none';\
document.body.innerHTML ='<div id=\"container\"><h1>MQTT Settings</h1><p style=\"text-align:center; margin: 2rem 0; color: #00d9ff;\">Settings saved! Rebooting...</p></div>';\
setTimeout(\"location.href = '/'\", 10000);\
}\
</script>"

/* Uses: %s (enable), %s (disco), %s (uri), %s (user), %s (pass), %d (intv) */
#define CONFIG_CHUNK_MQTT "\
<div class='section'>\
<form action='' method='GET'>\
<input type='hidden' name='mqtt_save' value='1'/>\
<table>\
<tr><td>Enable MQTT</td><td>\
<label><input type='checkbox' name='mqtt_en' value='1' %s></label>\
</td></tr>\
<tr><td>HA Discovery</td><td>\
<label><input type='checkbox' name='mqtt_disco' value='1' %s></label>\
</td></tr>\
<tr><td>Broker URI</td><td><input type='text' name='mqtt_uri' value='%s' placeholder='mqtt://192.168.4.2:1883'/></td></tr>\
<tr><td>Username</td><td><input type='text' name='mqtt_user' value='%s' placeholder='Optional'/></td></tr>\
<tr><td>Password</td><td><input type='password' name='mqtt_pass' value='%s' placeholder='Optional'/></td></tr>\
<tr><td>Publish Interval</td><td><input type='number' name='mqtt_intv' value='%d' min='5' max='3600'/></td></tr>\
